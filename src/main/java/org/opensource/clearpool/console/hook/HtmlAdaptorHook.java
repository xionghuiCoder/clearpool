...
public class HtmlAdaptorHook extends HtmlAdaptorServer {
  ...
  private static CommunicatorServer initServer(MBeanServer server) {
    ...
    CommunicatorServer communicatorServer =
        new HtmlAdaptorHook(MBeanFacade.console.getPort(), infoList.toArray(new AuthInfo[0]));
    try {
  ...
  /**
   * Set fields to stop it.Note:HtmlAdaptorServer didn't provided useful interface to stop it,so we
   * use reflection to get it.
   */
  public static void stop(CommunicatorServer communicatorServer) {
    try {
      Field field = CommunicatorServer.class.getDeclaredField("stopRequested");
