...
class TransactionImpl implements Transaction {
  private static final PoolLog LOG = PoolLogFactory.getLog(TransactionImpl.class);
  ...
  private void tryEndResource() {
    for (ResourceCarry carry : this.resList) {
      try {
        ...
        LOG.error("can't end XA: (error code = " + e.errorCode + ") ", e);
      }
    }
  }
  ...
  private boolean tryPrepared() {
    ...
    for (ResourceCarry carry : this.resList) {
      try {
        ...
        LOG.error("can't prepare XA: (error code = " + e.errorCode + ") ", e);
  ...
  private void tryCommit() throws HeuristicMixedException, SystemException {
    ...
    while (itr.hasNext()) {
      ...
      try {
        ...
        LOG.error("can't commit XA: (error code = " + e.errorCode + ") ", e);
        carryTemp = carry;
        break;
      }
  ...
  private void rollbackMixed(ResourceCarry carryTemp, Iterator<ResourceCarry> itr)
    ...
    try {
      ...
      LOG.error("can't roll back XA: (error code = " + e.errorCode + ") ", e);
  ...
  public boolean enlistResource(XAResource xaresource) throws RollbackException, SystemException {
    ...
    try {
      ...
      LOG.error("can't start XA: (error code = " + e.errorCode + ") ", e);
      return false;
    }
    return true;
  }
