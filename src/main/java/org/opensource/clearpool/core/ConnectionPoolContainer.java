...
class ConnectionPoolContainer {
  ...
  /**
   * It carry the pool,and it make sure the pool should just be loaded one time.
   */
  private static volatile Map<String, ConnectionPoolManager> poolMap =
   ...
   */
  static ConnectionPoolContainer load(String path, Map<String, ConfigurationVO> cfgMap) {
    if (cfgMap == null) {
  ...
  private static void checkPoolName(Set<String> nameSet) {
    ...
    for (String name : nameSet) {
      ...
      if (poolMap.get(name) != null) {
        throw new ConnectionPoolException("the pool " + name + " had been loaded");
      }
    }
  }
