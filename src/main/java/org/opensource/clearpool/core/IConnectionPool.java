...
/**
 * This is the common interface of the pool, every poolImpl should implements it so that they can
 * realize the three method: getConnection(),closeAll() and close(String name).
 * 
 * @author xionghui
 * @date 26.07.2014
 * @version 1.0
...
public interface IConnectionPool {
   ...
   * 
   * Note:if the default path had been init and not removed,it will throw a exception.
   */
  public void init();
   ...
   * Init pool by vo.
   * 
   * Note:if the vo's alias had been init and not removed,it will throw a exception.
   */
  ...
  /**
   * Close the database and connection in the pool which has the name of {@code name}.
   * 
   * Note:the method does nothing if the name is not existed or the name of the database has been
   * closed.
   * 
   * @param name is the pool's name
   */
